...
2.31_3 (2010/04/25)
    [蟲繕]
    ・パーセントを含むディレクトリに chdir できないため、関数・source 文
      以外では %0～%9 を空文字へ変換しないようにした。
    ・%% を % へ置換するようにした。
    ・4GB超のファイルのサイズが ls で正しく表示されない問題を修正
    ・ヒストリファイルが開けなかった時、終了時にエラー終了する問題を修正.
    ・option savehist 定義時に、Ctrl-Z で終了すると、空行が入ってしまう
      問題を修正
...
2.31_1 (2010/03/30)
  (Windows)
    [蟲繕]
    ・1文字入力のために変更したコンソールモードを元に戻していなかった箇所を修正
       (Python,Luaといった言語のインタラクティブモードが動作しなくなってしまっていた)

2.31_0 (2010/03/29)
  (nyacus) NYAOS-3000 での修正のバックポート(Thanks to lpproj/save)
    ・「澤」などの漢字入力と特殊キー入力の両立
